...
# Requirements (for running PRONTO locally, see below for Docker/Singularity)
...
3. Run Script/PRONTO.py                                                          
   ...
   This script is a tool used to generate the patient report based on the TSO500 analysis results and the personal information from the clinical data in In/InPreD_PRONTO_metadata.txt,
   and update the TSOPPI results into the file Out/InPreD_PRONTO_metadata_tsoppi.txt when the reports are generated.          
   This script can also fill in the patient personal information into the clinical data file with the MTF files under the folder In/MTF/. (This function currently is only used by OUS)                                                                
...
"$tsoppi_data" is the path of your local TSOPPI results, which contain all runs of TSOPPI data (not the folder for individual runs).            
